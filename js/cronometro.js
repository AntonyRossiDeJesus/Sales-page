...
let countdownDuration = 12 * 60 * 60; // 12 hours in seconds
...
const saveCountdown = () => {
  const now = Math.floor(Date.now() / 1000);
  localStorage.setItem("countdown", now - (12 * 60 * 60 - countdownDuration));
};

const updateTimer = () => {
  let hours = Math.floor(countdownDuration / (60 * 60));
  let minutes = Math.floor((countdownDuration % (60 * 60)) / 60);
  let seconds = Math.floor(countdownDuration % 60);
  ...
  if (countdownDuration > 0) {
    ...
    countdownDuration = 12 * 60 * 60; // Reset to 12 hours
  }
};
